...
function parsePackage($html){
  ...
  scraped = {
    ...
    name: trim($html.find('.name, td:first-child').text()),
    priceDiscount: trim($html.find('.price-discount').text()),
  };
  var $newHtml = $html.remove('.price-discount');
  scraped.price = trim($newHtml.find('.price, td:nth-child(2)').text().replace(scraped.priceDiscount, ''));

  // showModal('/package/:id'), this strips non-number chars.
  scraped.id = $html.find('.image a, td:nth-child(3) a').attr('onclick').replace(/\D/g, '');
